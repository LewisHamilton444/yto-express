...
2. **Clean empty states, gated by `VITE_DEMO_MODE`**: with the flag off, 0 records renders the clean empty-state view — zero synthetic or mock injections anywhere in the client, and unreachable backends display an error banner, never simulated data. This was the unconditional behavior before 2026-09-22; see point 4 for the current default.
3. **Database specification**: Pinned to Atlas DB `/test` (`server/.env` `MONGO_URI`); operational collections clean.
4. **`VITE_DEMO_MODE` fallback — ON by default as of 2026-09-23 (pre-launch phase)**: a build-time-only flag (`import.meta.env.VITE_DEMO_MODE !== '0'`), read once in `src/services/api.js`. No mobile app is connected yet, so every real endpoint below is currently empty regardless — defaulting the flag on means the deployed UAT site (`yto-express-web.onrender.com`) shows populated data with **no Render dashboard step required**. Set `VITE_DEMO_MODE=0` at build time to force it off (e.g. once cutting over to real production). Behavior while on: `apiFetch` transparently swaps an **empty** GET response on a known collection route (`/customers`, `/sellers`, `/riders`, `/parcels`, `/issues`, `/accounts`, `/parcel-locations`, `/activity-log`, `/notifications`, `/dashboard/stats`) for the fixture set in `src/services/demoFixtures.js` — the same linked dataset `scripts/qa/seedServer.mjs` serves locally (both import it; one dataset, not two that can drift). Mutations (POST/PUT/PATCH/DELETE) are never intercepted, and **a real non-empty response is never overridden** — the moment any endpoint has real rows synced from the mobile app, that endpoint's real data wins automatically and permanently, no redeploy needed. `AnalyticsDashboard.jsx` renders a persistent amber "Demo Data" banner across every page whenever the flag is on, so fabricated rows are never mistaken for real ones. **Before the real-production launch, set `VITE_DEMO_MODE=0` in Render and redeploy** — no other code change or database cleanup needed (nothing here was ever written to Mongo). **Known exception:** `src/ActivityLog.jsx` was separately hardcoded (2026-09-23, outside this mechanism) to always return 4 fixed mock rows from `fetchEvents()` — unlike every route above, it does **not** hand off to real data once real activity exists and is not gated by `VITE_DEMO_MODE` or covered by the banner. Needs its own fix before real launch.
