...
- `server/backfill_parcel_account_category.js` — one-time gated remediation (`--apply` requires `BACKFILL_CONFIRM=YES`; dry run by default) that stamps `accountCategory` onto pre-fix parcels using the same heuristics as `ManageParcels.normalizeParcel`. **2026-09-09 run: found 0 parcels lacking the field** (the Atlas DB used by the deployed backend currently holds 0 parcel documents), so the backfill was a verified no-op; the script stays for future data imports. NOTE: the connection string in `.env` has no database name in the path, so Mongoose resolves to the `test` db on the Atlas cluster — the data-bearing db is `test` (the empty `yto_express` db also exists); consider naming the db explicitly in `MONGO_URI` to avoid ambiguity.
