...
- Dashboard content order (2026-09-18): operational sections render first — anchor KPIs, then the canvas grid (Parcel Volume 2.2fr paired with the Parcel Operations rail 1fr), then a balanced `ed-secondary-row` (Rider Performance 1.6fr / Rider Activity 1fr); `PeakAlertBanner` and `ConnectionHistoryChart` are secondary diagnostics rendered after both rows, not above them. Single-column stack below 1080px.
