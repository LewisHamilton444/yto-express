...
Before applying any rule here, read YTO_ADAPTATIONS.md - this repo documents ESCALATE exceptions where the existing design system outranks a no-slop rule.
...
- No `Trebuchet MS` font stacks; no `Arial` outside print/export stylesheets (`Segoe UI` is this repo's existing app font - see `YTO_ADAPTATIONS.md`)
